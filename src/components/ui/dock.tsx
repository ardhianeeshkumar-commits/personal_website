...
const Dock = ({ className = '', items = [], position = 'bottom' }: DockProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        `flex ${dockLayout} items-center gap-2 sm:gap-3 md:gap-4 rounded-full bg-zinc-800/70 border border-white border-opacity-5 ${className}`,
        className
      )}
    >
        ...
        return (
          <div key={index} className="relative">
            ...
            <motion.div
              ref={ref}
              style={{ scale }}
              className={`aspect-square h-14 w-14 rounded-full flex items-center justify-center transition-colors duration-300 ${hovered ? item.hoverBgColor || 'bg-zinc-700' : item.defaultBgColor || 'bg-zinc-700'}`}
